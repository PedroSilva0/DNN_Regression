...
def create_model():
	model = Sequential()
	model.add(Dense(24, input_dim=12, activation="relu", kernel_initializer="uniform"))
...
def ciclo_completo():
	...
	print('Normalized Train Score: %.10f NRMSE  (%.10f%% NRMSE)' % (nmrse,nmrse*100))
	...
	print('Normalize Test Score: %.10f NRMSE  (%.10f%% NRMSE)' % (nmrse,nmrse*100))
